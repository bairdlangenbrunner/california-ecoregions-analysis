...
root_dir = '/glade/collections/cdg/data/cesmLE/CESM-CAM5-BGC-LE/atm/proc/tseries/daily/QBOT/'
file_name = 'b.e11.B20TRC5CNBDRD.f09_g16.002.cam.h1.QBOT.19200101-20051231.nc'
...
lon_pcolormesh = numpy.zeros(qbot_lon.size+2)
lon_pcolormesh[1:-1] = qbot_lon
lon_pcolormesh[0] = qbot_lon[0]-numpy.diff(qbot_lon)[0]
lon_pcolormesh[-1] = qbot_lon[-1]+numpy.diff(qbot_lon)[-1]
lon_pcolormesh_midpoints = lon_pcolormesh[:-1]+0.5*(numpy.diff(lon_pcolormesh))

lat_pcolormesh = numpy.zeros(qbot_lat.size+2)
lat_pcolormesh[1:-1] = qbot_lat
lat_pcolormesh[0] = qbot_lat[0]-numpy.diff(qbot_lat)[0]
lat_pcolormesh[-1] = qbot_lat[-1]+numpy.diff(qbot_lat)[-1]
lat_pcolormesh_midpoints = lat_pcolormesh[:-1]+0.5*(numpy.diff(lat_pcolormesh))

################################################################################

# not dependent on actual data
latlon_index_combos = numpy.array([i for i in itertools.product(range(qbot_lat.size),range(qbot_lon.size))])
lat_polygon_hi_list = []
lon_polygon_hi_list = []
lat_polygon_lo_list = []
lon_polygon_lo_list = []
...
for latlon in latlon_index_combos:
    ...
    lon_list.append(qbot_lon[latlon[1]])
    lat_list.append(qbot_lat[latlon[0]])
...
intersecqbot_TF_list = []
for box in polygon_boxes:
    intersecqbot_TF_list.append(ecoregion_series_geom_4326.intersects(box))
latlon_index_combos_intersect = latlon_index_combos[intersecqbot_TF_list]

# calculates percent area overlap
#for box in polygon_boxes[intersecqbot_TF_list]:
#    print('{:.2f}'.format(ecoregion_series_geom_4326.intersection(box).area/box.area))

# create mask of NaNs and fractional values
mask_and_weights = numpy.zeros((qbot_lat.size,qbot_lon.size))*numpy.nan
for i,latlon in enumerate(latlon_index_combos_intersect):
    ...
    polygon_boxes[intersecqbot_TF_list][i]).area/polygon_boxes[intersecqbot_TF_list][i].area
...
for file_name in file_name_list[:2]:
    ...
    qbot_data_array = xarray.DataArray(weighted_ecoregion_qbot_mean, coords=[time_data], dims=['time'])
    ...
    data_set = xarray.Dataset({'TS': (['time'], qbot_data_array)}, coords={'time': (['time'], time_data)})
